...
CREATE TABLE TRAVEL_IMAGES (
    travel_image_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- 여행 이미지 ID (자동 생성, 기본 키)
    travel_id BIGINT NOT NULL,                                   -- 해당 이미지가 속한 여행 ID (외래 키, NOT NULL)
    location_detail_id BIGINT NOT NULL,                               -- 해당 이미지가 속한 여행 계획 상세 정보 ID (외래 키, NOT NULL)
    file_name VARCHAR(255) NOT NULL,                               -- 파일 이름 (NOT NULL)
    file_path VARCHAR(255) NOT NULL,                               -- 파일 저장 경로 (NOT NULL)
    file_size BIGINT,                                              -- 파일 크기 (NULL 허용)
    mime_type VARCHAR(100),                                         -- 파일 MIME 타입 (NULL 허용)
    description VARCHAR(255),                                         -- 이미지에 대한 설명 (NULL 허용)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                   -- 생성 일시 (기본값: 현재 타임스탬프)
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP                    -- 수정 일시 (기본값: 현재 타임스탬프)
);
ALTER TABLE TRAVEL_IMAGES ADD CONSTRAINT fk_travel_images_travel_id FOREIGN KEY (travel_id) REFERENCES TRAVEL(travel_id);
ALTER TABLE TRAVEL_IMAGES ADD CONSTRAINT fk_travel_images_location_detail_id FOREIGN KEY (location_detail_id) REFERENCES LOCATION_DETAIL(location_detail_id);
CREATE INDEX idx_travel_images_travel_location ON TRAVEL_IMAGES (travel_id, location_detail_id);
